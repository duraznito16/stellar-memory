...
test('understands OpenZeppelin guard macros and derived upgradeability', () => {
  // OpenZeppelin's Stellar library moves both facts out of the function body:
  // `#[only_owner]` performs the authorization check, and the upgrade entry
  // point is generated by `#[derive(Upgradeable)]`. Reading the body alone
  // reports audited code as an unauthenticated, non-upgradeable contract —
  // two false statements about it.
  const src = `
...
impl MyToken {
    ...
    pub fn mint(e: Env, to: Address, amount: i128) {
        e.storage().instance().set(&DataKey::Supply, &amount);
    }

    #[only_owner]
    #[when_not_paused]
    pub fn set_metadata(e: Env, uri: String) {
...
`;
  const contract = analyseRustFile('oz.rs', src).contracts[0]!;
  assert.equal(contract.upgradeable, true, 'the derive macro makes it upgradeable');

  const fn = (name: string) => contract.functions.find((f) => f.name === name)!;

  assert.deepEqual(fn('mint').guards, ['only_owner']);
  assert.equal(fn('mint').requiresAuth, true, 'the macro is the authorization');
  assert.equal(fn('mint').authSubjects[0]?.origin, 'guard-macro');

  // A state guard restricts when a function runs, not who runs it.
  assert.deepEqual(fn('set_metadata').guards, ['only_owner'], 'when_not_paused is not access control');

  // An ungated writer is still ungated.
  assert.deepEqual(fn('set_fee').guards, []);
  assert.equal(fn('set_fee').requiresAuth, false);
});

test('recognises UpgradeableInternal even when the derive is not adjacent', () => {
  const src = `
use soroban_sdk::{contract, contractimpl, Address, Env};
#[contract]
pub struct Vault;
#[contractimpl]
impl Vault {
    pub fn balance(e: Env) -> i128 { 0 }
}
impl UpgradeableInternal for Vault {
    fn _require_auth(e: &Env, operator: &Address) { operator.require_auth(); }
}
`;
  const contract = analyseRustFile('v.rs', src).contracts[0]!;
  assert.equal(contract.upgradeable, true);
});
